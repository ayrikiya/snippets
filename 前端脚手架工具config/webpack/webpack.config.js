...
module.exports = {
    ...
    optimization: {
        splitChunks: { // 即使splitChunks为空对象，也会加载一段默认配置，具体见 splitChunksPlugin官网介绍。
            ...
            cacheGroups: { 
            // 同步代码走chacheGroups寻找配置，下面配置了test框定模块范围，那么即使直接写的模块导出引入，小于minSize 它也不会被打包进vendor 因为不再node_module中，那么这种自己写的模块代码会走cacheGroups的default配置; 
            // cacheGroup可以继承和/或覆盖来自 splitChunks.*的任何选项。但是 test、priority 和 reuseExistingChunk 只能在缓存组级别上进行配置。将它们设置为 false以禁用任何默认缓存组。
                ...
                appVendor: {
                    name: "app-vendor",
                    chunks: (chunk) => {
                        return chunk.name === "app"; // 该条group配置表示 一般在多入口情况下 entry名是app的统一打包到app-vendor的bundle里，当然还可以写test规则细化区分。
                    }
                },
                vendors: {
